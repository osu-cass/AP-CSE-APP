...
storiesOf('PDFLink/Document', module)
  .add('ELA Document', () => <PDFViewer style={iframeStyle}>{createDocument(elaDocumentFull)}</PDFViewer>)
  .add('ELA document partial', () => <PDFViewer style={iframeStyle}>{createDocument(elaDocumentPartial)}</PDFViewer>)
  .add('Renders with Test data', () => <PDFViewer style={iframeStyle}>{createDocument({claim: ClaimMe, taskModels:ClaimMe.target[0].taskModels, renderEntireTarget: true})}</PDFViewer>);
...
export const TaskModelComponent = async ({ taskModels, stems }: TaskModelComponentProps) => {
  const taskModelsPromises = taskModels.map(async (taskModel: ITaskModel, idx: number) => (
    <View key={`${idx}-${idx}`}>{await TaskModel({ taskModel, stems })}</View>
  ));

  const taskModelsJsx = await Promise.all(taskModelsPromises);
